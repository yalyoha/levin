...
class Image {
    ...
    static function getImg($width = 400, $height = 300, $crop = true, $quality = 80) {
        $crc32 = self::$crc32;
        $result = Db::query("SELECT image FROM images WHERE crc32=$crc32 LIMIT 1");
        $data = $result->fetch_assoc();
        if ($result->num_rows > 0) return self::thumb($data['image'], $width, $height, $crop, $quality);
    ...
    static function src($alias, $width, $height, $crop = true, $quality = 80) {
        self::alias($alias);
        echo self::getImg($width, $height, $crop, $quality);
    }

    // thumb maker

    static function thumb($src, $width, $height, $crop, $quality = 80) {
        $path = DROOT . $src;
        $info = pathinfo($path);
