...
class Image {
    ...
    static function setImg($alias, $data) {
        ...
        if (Db::conn()->query("UPDATE images SET image='$data' WHERE crc32=$crc32") === true) return Db::conn()->affected_rows;
    ...
    static function insertImg($alias, $crc32, $data, $width, $height, $crop) {
        ...
        $data = strip_tags($data);
        $data = Db::conn()->escape_string($data);
        if (Db::conn()->query("INSERT INTO images (crc32, alias, image) VALUES ('$crc32', '$alias', '$data')") === true) return self::getImg($alias, $width, $height, $crop);
        else  die("Error: " . Db::conn()->error);
    }
    ...
    static function here($alias, $width, $height = null, $attrClass = '', $attrAlt = '', $crop = true) {
        ...
        $crc32 = sprintf("%u", crc32($alias));
        if (!isset($crc32)) return;
        $adminBtn = '<span class="admin" data="/admin/edit.php?what=img&alias=' . $alias . '&w=' . $width . '&h=' . $height . '&c=' . $crop . '">[' . $alias . ']</span>';
